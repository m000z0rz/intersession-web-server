...
var MONGOSTORE = module.exports = function MongoStore(options, callback) {
  ...
  function getCollection(db, callback) {
    _collectionName = (options.collection || _defaults.collection)
  ...
  if (options.url) {
    ...
    options.db = new _mongoskin.Db(url.pathname.replace(/^\//, ''),
                              new _mongoskin.Server(url.hostname || _defaults.host,
                                               parseInt(url.port) || _defaults.port));
  }

  if (options.server_config) {
    server_config = options.server_config;
    db = new _mongoskin.Db(options.dbname || _defaults.dbname, server_config);
  }
  ...
  if (options.db) {
    ...
    if(!server_config) {
        //only got database, need to ge collection
        _collectionName = (options.collection || _defaults.collection);
        _db = db;
    }
  }
...
MONGOSTORE.prototype.get = function (sid, cb) {
  ...
  _db.collection(_collectionName).findOne({_id: sid}, function (err, data) {
    try {
      if (data) {
...
MONGOSTORE.prototype.set = function (sid, sess, cb) {
  ...
  //_collection.update({_id: sid.toString()}, update, {upsert: true}, function (err, data) {
  _db.collection(_collectionName).update({_id: sid.toString()}, update, {upsert: true}, function (err, data) {
...
MONGOSTORE.prototype.destroy = function (sid, cb) {
  ...
  _db.collection(_collectionName).remove({_id: sid}, _default(cb));
};
...
MONGOSTORE.prototype.length = function (cb) {
  ...
  _db.collection(_collectionName).count({}, _default(cb));
};
...
MONGOSTORE.prototype.clear = function (cb) {
  ...
  _db.collection(_collectionName).drop(_default(cb));
};
